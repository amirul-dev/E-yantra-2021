...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  for i=1:length(solutions)
    ans = subs(jacob, [x1,x2], [solutions(i,1), solutions(i,2)]);
    jacobian_matrices(i)=double(ans);
  endfor
  ############################################################  
endfunction
...
function [eigen_values stability] = check_eigen_values(x_1, x_2, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    flag = 1;
    ################## ADD YOUR CODE HERE ######################
      eigen = eig(jacobian_matrices{k});
      eigen_values(k) = eigen;
      if real(eigen_values{k}(1))>0 || real(eigen_values{k}(2))>0
        flag=0;
      endif
    ############################################################
    if flag == 1
